...
g_char_descriper gc_add_Character_to_context(g_character *player)
{
    ...
    for (int i = 0; i < MAX_PLAYERS; i++) {
        if (g_player_context[i] == NULL) {
            ...
            player_count++;
            u_Log_Debug("GameCore: Player added to context at index %d.\n", i);
...
g_item_descriper gc_add_Item_to_context(g_item *item)
{
    if (item_count >= MAX_ITEMS) {
        ...
        gc_error_code = ERROR_MAX_ITEMS_REACHED; // Set error code for maximum items reached
        return NULL; 
    }
    ...
    for (int i = 0; i < MAX_ITEMS; i++) {
        if (g_item_context[i] == NULL) {
            ...
            u_Log_Debug("GameCore: Item added to context at index %d.\n", i);
            return i; // Item added successfully
        }
    }
